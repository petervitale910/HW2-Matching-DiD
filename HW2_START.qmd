...
except the “storms” we care about are electoral swings that might follow
...
nearby), and those local opponents may “send a signal” at the ballot box
...
spillovers when estimating each group’s treatment effect.
...
(yes). The covariates are rather balanced, remaining pretty close throughout. The only covariate with any imballance is log_pop_dencity which has by far the biggest difference between treatment and control.
...
*Response:* Before matching the smallest `Std. Mean Diff.` is `p_uni_degree` at -.05 and the largest is `log_pop_denc` at -.88. This changes drastically when matched and actually becomes a -.0093. After treatment the largest `Std. Mean Diff.` is `log_median_inc` at .0002 and the smallest is `log_pop_denc`. But in general all the `Std. Mean Diff.`'s all become **tiny** which is great! 
...
*Response:* The low p-value suggests the relationship between proposed turbine development and political change is (statistically significantly) unlikely to be random. This was done after the matching, which adds rigor to our estimate. However there may be missing some omitted variables which means we may have not found a true causal relationship. 
...
*Response:* ATT represents the treatment effect of those in precints who received a turbine, which were the 354 treated units (matches). The reason that we do not use ATE is if treatment is randomly assigned, but in our case we use the treatment effect for the treated as it is not random or equal. 
...
*Response:* The results may differ from Stokes (2015) because this model is estimated on a small random subsample of precincts rather than the full dataset, which reduces precision and may yield estimates that are less stable or representative of the full sample. 
...
*Response:* The implementation of an operational turbine creates a 9 percentage point drop in liberal voting, compared to what precincts which have not received turbines. This expectation is statistically significant, meaning it is likely not due to random chance. 
